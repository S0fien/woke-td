...
export class Player extends Actor {
  ...
  override async onInitialize(engine: Engine) {
    ...
    // 3. Deferring logic to run time ins tead of constructor time
    // 4. Lazy instantiation
    // await Resources.Sheets.Girl.load()
    this.graphics.add(Resources.Sheets.Girl.toSprite());
    this.graphics.use(Resources.Sheets.Girl.toSprite());
    // lesrpite.getSprite(0, 0, {
    //   width: 20,
    //   height: 20,
    //   origin: vec(0, 0),
    // })
    ...
    this.graphics.add(
      Resources.Weapon.Sword.toSprite({
        width: 20,
        height: 20,
        sourceView: {
          x: 0,
