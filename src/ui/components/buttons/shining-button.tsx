...
export default function ShiningButton({
  ...
  type,
  ...props
}: React.HTMLProps<HTMLButtonElement> & { type?: 'button' | 'submit' | 'reset' | undefined }) {
  return (
    <button
      type={type}
      className="group border-opacity-0 hover:border-opacity-100 cursor-pointer rounded-xl border-4 border-violet-800 bg-transparent p-1 transition-all duration-500"
      {...props}
    >
